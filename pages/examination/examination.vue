<template>
  <view class="box-hg">
    ...
    <view class="cu-modal drawer-modal justify-start"
          :class="endModalName=='DrawerModalL'?'show':''"
          @tap="endHideModal">
      <view class="cu-dialog basis-lg"
            @tap.stop=""
            :style="[{top:45+'px',height:'calc(100vh - ' + 45 + 'px)'}]">
        <view class="flex justify-around flex-wrap">
          <view class="arrow display-inline"
                v-for="(item,index) in topicsList.length"
                @click="selectTopic(index)"
                :key="index">
            <view class="content padding margin-top-sm "
                  :class="answerTrueFalse[index]==null?'bg-grey':'bg-blue'">
        ...
        <view class="cu-bar bg-white justify-end">
          <view class="action">
            <button class="cu-btn line-blue text-blue"
                    @tap="handInPapersModel = false">关闭</button>
            <button class="cu-btn bg-blue margin-left"
                    @tap="handInPapersTrue">提交</button>
          </view>
        </view>
      </view>
    </view>
    ...
    <template v-if="!loadModal">
      ...
      <view class="box-buttom ">
        <view class="flex justify-between">
          <!-- 统计答对数 -->
          <view @click="handInPapers"
                class=" bg-olive padding-lr-xl padding-tb-sm radius">交卷</view>
          <!-- 统计未答数 -->
          <view @click="clickIndex"
