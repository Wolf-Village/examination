<template>
	<view style="margin: auto">
		<view class="jincehng-input-box">
			<input class="jincheng-input" type="text" value="" placeholder="输入关键字" v-model="searchcontent" />
			<view class="jincheng-search" @click="search()">
				搜索
			</view>
		</view>
		<view style="margin: auto;">
			...
			<view class="margin1" v-show="isSistory">
				...
				<view class="flex flex-wrap">
					<view class="jincehng-ti1 jincheng-font bg-grey round margin-lr-sm margin-top " v-for="(item,index) in historyList"
					 :key='index' @click="history(item)">{{item}}</view>
				</view>
			</view>
			<view class="jincehng-xuan" style="    margin: 20px auto;
    width: 95%;" v-for="(item,index) in dataa" :key='item+index'>
				<view class="jincehng-ti jincheng-font" @click="todetails(item.id)">
					<view v-if="item.type=='radio'" style="margin-bottom: 1vh;font-weight: 700;">[单选]</view>
					<view v-if="item.type=='checkbox'" style="font-weight: 900;">[多选]</view>
					<view v-if="item.type=='judge'" style="font-weight: 900;">[判断]</view>
					{{item.name}}
				</view>
